...
byte x_shares[160*(N-1)]; //Shares of x used as part of pre-processing.  496 *(N2-1) fpr PRESENT
...
void htable_r(int n,int count,int choice)//htable_r(n,i);//1 for normal 0 for increasing
{
  ...
  {
    shift_tab(x_shares[count*(N-1)+i],Tp,count);

    if(choice==NPRG)
        loc_refresh_table_r(Tp,x_shares[count*(N-1)+i],n,i,count);

    else if(choice==IPRG)
        loc_refresh_table_r_inc(Tp,x_shares[count*(N-1)+i],n,i,count);
...
void read_htable_m(byte a,byte *b,int n,int count)
{
	...
	for(int j=1;j<n;j++)
		b[j]=get_mprg_lr(val+j-1,n,pre_val+a);
