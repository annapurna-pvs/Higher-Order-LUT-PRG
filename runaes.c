...
void keyexpansion_share1(byte key[16],int n)
{
  byte w[176];
  keyexpansion(key,w);
...
void addroundkey_share2(byte stateshare[16][N2],int round,int n)
{
  int i,j;
  for(i=0;i<16;i++)
    for(j=0;j<n;j++)
...
void shiftrows_share2(byte stateshare[16][N2],int n)
{
  byte m;
  int i;
  for(i=0;i<n;i++)
...
void mixcolumns_share2(byte stateshare[16][N2],int n)
{
  byte ns[16];
  int i,j;
  for(i=0;i<n;i++)
...
void aes_share_subkeys1(byte in[16],byte out[16],int n,void (*subbyte_share_call)(byte *,int,int),int loc)
{
  int i;
  int round=0;
  ...
  addroundkey_share2(stateshare,0,n);

  for(round=1;round<10;round++)
  {
    subbytestate_share1(stateshare,n,subbyte_share_call,round-1);
    shiftrows_share2(stateshare,n);
    mixcolumns_share2(stateshare,n);
    addroundkey_share2(stateshare,round,n);
  }

  subbytestate_share1(stateshare,n,subbyte_share_call,round-1);
  shiftrows_share2(stateshare,n);
  addroundkey_share2(stateshare,10,n);
  for(i=0;i<16;i++)
  {
...
void run_aes_share_rprg_table(byte in[16],byte out[16],byte key[16],byte *outex,int n,void (*subbyte_share_call)(byte *,int,int),int nt,int rprg)
{
  //int prgcount,i;
   int i;
  keyexpansion_share1(key,n);

  for(i=0;i<nt;i++)
  {
    aes_share_subkeys1(in,out,n,subbyte_share_call,1);//NOLOC is Zero and LOC is 1
  }
...
void run_aes_share_mprg_table(byte in[16],byte out[16],byte key[16],byte *outex,int n,void (*subbyte_share_call)(byte *,int,int),int nt,int ni)
{
  ...
  for(int i=0;i<nt;i++)
    aes_share_subkeys1(in,out,n,subbyte_share_call,1);

}

int rprg_table(int n)
{
	int loc=1;
	loc=2*(n-1);
	return (n-1)*loc;
...
int main()
{
		...
		byte key[16];
		int choice=IPRG;
		int type=MPRG; //Type of PRG to generate randoms, either robust or multiple
		//int base;
