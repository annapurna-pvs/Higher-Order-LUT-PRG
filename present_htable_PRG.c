...
byte p_x_shares[496*(N-1)]; //Shares of x used as part of pre-processing.  496 *(N2-1) fpr PRESENT
...
void loc_refresh_table_inc_m_present(byte *Tp,byte a,int n,int ind,int count)
{
    ...
    for(int k=0;k<P_TSIZE;k++)
    {
			...
			{
                int val=(ind*(ind+1))/2;
                byte tmp=get_mprg_lr(val+i,n,pre_val+k)%16;
                Tp[k]=Tp[k] ^ tmp;
			}
            ...
            if(ind>0)
            {
                ...
                {
                    int val=((ind-1)*ind)/2;
                    byte tmp=get_mprg_lr(val+i,n,pre_val+(k^a))%16;
                    Tp[k]=Tp[k] ^ tmp;
                }
            }
...
void htable_m_present(byte n,int count)
{
    unsigned int t,temp;
    byte Tp[P_TSIZE];
...
void gen_t_forall_present(byte n)
{
...
void read_htable_inc_m_present(byte a,byte *b,int n,int count)
{
    ...
    for(int j=1;j<n;j++)
			b[j]=get_mprg_lr(val+j-1,n,pre_val+a)%16;
...
void subbytestate_share_prg_present(byte *stateshare[8],int n,void (*subbyte_share_call)(byte *,int,int),int round)
{
  unsigned int i,j,k;
